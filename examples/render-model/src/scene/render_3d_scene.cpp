...
void Render3DScene::onEnter() {
    ...
    std::optional<bolt::ShaderProgram> shader = bolt::ShaderProgram::build("assets/shaders/vshader.glsl", "assets/shaders/fshader.glsl");
...
void Render3DScene::update(uint64_t deltaTime) {
    ...
    m_camera.m_view = glm::lookAt(
        glm::vec3(x, 0, z), // camera is at (4, 3, 3) in world space
